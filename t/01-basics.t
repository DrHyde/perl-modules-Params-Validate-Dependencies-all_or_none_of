...
foreach my $one (qw(alpha beta gamma)) {
  ...
  dies_ok(foo($one => 1), "only one ($one) of three, validation failed");
  dies_ok(foo($two[0] => 1, $two[1] => 1),
...
dies_ok(foo(alpha => 1), "validation fails (alpha only), code-ref");
dies_ok(foo(beta  => 1), "validation fails (beta  only), code-ref");
dies_ok(foo(gamma => 1), "validation fails (gamma only), code-ref");
